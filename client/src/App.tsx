...
export default function App() {
  ...
  return <main>
    ...
    <section className="list" aria-live="polite">{loading ? <p>Loading calls…</p> : calls.length === 0 ? <p className="muted">No calls yet.</p> : calls.map(call => <article className="panel" key={call.id}><div className="card-head"><h3>{call.recordType === 'incident' ? 'Incident call' : call.recordType === 'information' ? 'Information call' : 'Unclassified call'}</h3><span>{call.processingStatus}</span></div><p><strong>{call.externalCallId}</strong> · {date(call.createdAt)} · ID: {call.id}</p><p className="transcript">{call.transcript}</p>{call.processingError && <p className="error">Processing error: {call.processingError}</p>}<ReportDetails report={call.report} /></article>)}</section>
